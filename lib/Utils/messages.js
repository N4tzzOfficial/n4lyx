...
const generateWAMessageContent = async (message, options) => {
    ...
    const buildAdvancedInteractive = () => {
        const buttons = message.buttons || message.interactiveButtons
    ...
    if ('text' in message) {
        ...
        (_p = message.poll).selectableCount ||= 0
        (_q = message.poll).toAnnouncementGroup ||= false
        ...
        m.messageContextInfo = {
            messageSecret: message.poll.messageSecret || crypto.randomBytes(32)
        }
    ...
    const advancedInteractive = buildAdvancedInteractive()
    if (advancedInteractive) {
        m = advancedInteractive
    }
    ...
    if ('contextInfo' in message && message.contextInfo) {
        const [type] = Object.keys(m)
        m[type] = m[type] || {}
        m[type].contextInfo = message.contextInfo
    }
